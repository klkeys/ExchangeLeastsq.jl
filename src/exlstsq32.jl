...
@compat function exchange_leastsq!(
	...
	rss     = 0.0f0	# residual sum of squares || Y - XB ||^2
	...
	BLAS.gemv!('T', 1.0f0, X, res, 0.0f0, df)
	...
	for iter = 1:(max_iter)
		...
		for i = abs(r-window+1):r
			...
			l     = perm[i]
			betal = bvec[l]
			update_col!(tempn, X, l, n=n, p=p, a=1.0f0)	# tempn now holds X[:,l]
			...
			# > dotprods = get!(inner, l, BLAS.gemv('T', 1.0f0, X, tempn))
			if !haskey(inner, l)
				inner[l] = BLAS.gemv('T', 1.0f0, X, tempn)
			end
			...
			m = perm[k]
			update_col!(tempn2, X, m, n=n, p=p, a=1.0f0)	# tempn2 now holds X[:,m]
#			axpymbz!(res, betal, tempn, adb, tempn2, p=n)
			...
			if !haskey(inner, m)
				inner[m] = BLAS.gemv('T', 1.0f0, X, tempn2)
			end
			copy!(tempp, inner[m])
