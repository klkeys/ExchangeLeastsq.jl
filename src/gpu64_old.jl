...
#	nrmsq       :: DenseVector{Float64} = sumsq(x, shared=false, means=means, invstds=invstds, pids=pids), 
	n           :: Int = length(Y), 
	p           :: Int = size(X,2), 
...
#	p == length(nrmsq)    || throw(DimensionMismatch("length(bvec) != length(nrmsq)"))
	...
	rss     = oftype(zero(Float64), Inf)			# residual sum of squares || Y - XB ||^2
...
#			b   = nrmsq[l]
			b   = n	# sumabs2() of all columns is n since they are standardized on the fly! 
			a   = df[l] + betal*b
			adb = a / b
			k   = i
...
#				d   = nrmsq[idx] # sumabs2() of all columns is n since they are standardized on the fly! 
				d   = n 
...
#	nrmsq       :: DenseVector{Float64} = sumsq(x, shared=true, means=means, invstds=invstds, pids=pids), 
	tol         :: Float64              = 1e-4,
	max_iter    :: Int                  = 100, 
	window      :: Int                  = 20, 
	n           :: Int                  = length(y),
...
#								nrmsq   = SharedArray(abspath(norm2file), Float64, (p,), pids=pids)
...
#	end
	...
	devindices = 2*ones(Int, numfolds)
#	for i = 1:numfolds
#		devindices[i] += i % ndev
#	end
